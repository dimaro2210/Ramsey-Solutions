...
export function getSimulatedPrice(base: number, seed: number): number {
  const variance = base * 0.003;
  const offset = Math.sin(seed * 0.1) * variance + Math.cos(seed * 0.07) * variance * 0.5;
  return Math.round((base + offset) * 100) / 100;
}

export function getMarketData(tick: number): { crypto: Asset[]; stocks: Asset[] } {
  const crypto = BASE_CRYPTO.map((a) => {
    const newPrice = getSimulatedPrice(a.price, tick + a.symbol.charCodeAt(0));
    const change = Math.round((newPrice - a.price) * 100) / 100;
    const changePercent = Math.round((change / a.price) * 10000) / 100;
    return { ...a, price: newPrice, change, changePercent };
  });

  const stocks = BASE_STOCKS.map((a) => {
    const newPrice = getSimulatedPrice(a.price, tick + a.symbol.charCodeAt(0) + 50);
    const change = Math.round((newPrice - a.price) * 100) / 100;
    const changePercent = Math.round((change / a.price) * 10000) / 100;
    return { ...a, price: newPrice, change, changePercent };
  });
